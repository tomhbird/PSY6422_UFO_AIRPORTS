...
#display table of UFO sightings data
head(ufo_data_trim)
```
...
#display table of USA major airport data
head(airport_data_filtered)
```
...
### Creating a bar graph of the top 20 most common observed shapes of UFO's
```{r results = TRUE}
...
# Create a bar plot to visualize the frequency of top 20 UFO shapes
ggplot(ufo_shape_top20, aes(x = reorder(shape, count), y = count, fill = shape)) +
  geom_bar(stat = "identity", fill = "darkgreen", width = 0.7) +
  labs(title = "Most Common Shapes of UFOs in mainland USA",
       x = "Shape",
       y = "Frequency") +
  coord_flip() +
  scale_y_continuous(breaks = seq(0, max(ufo_shape_top20$count), by = 2000), limits = c(0, max(ufo_shape_top20$count) + 2000)) +
  theme_minimal() +
...
### #creating stacked barchart to show the most common shapes of UFO and their durations in mainland USA
```{r results = TRUE}
# Calculate quantiles
...
ggplot(combined_data, aes(x = reorder(shape, count), y = count, fill = duration_category)) +
